...
-- FIXED VERSION: Handles existing duplicates properly
...
BEGIN
    FOR dup_record IN SELECT * FROM duplicate_profiles LOOP
        ...
        -- Update votes to point to the kept profile (if votes table exists)
        BEGIN
            UPDATE votes
            SET wallet_address = dup_record.normalized_address
            WHERE LOWER(wallet_address) = dup_record.normalized_address;
        EXCEPTION
...
-- Step 7: Normalize wallet_address in votes table (if exists)
DO $$
BEGIN
    UPDATE votes
    SET wallet_address = LOWER(wallet_address)
...
-- This will fail if there are still duplicates, which means something went wrong above
DO $$
BEGIN
